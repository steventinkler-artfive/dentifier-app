...
export default function AssessmentDetail() {
  ...
  const handleAddNewVehicle = async (newVehicleData) => {
    ...
    try {
      await base44.entities.Assessment.update(assessment.id, {
        vehicles: [...(assessment.vehicles || []), newVehicleData]
      });
      await loadAssessmentDetails();
  ...
  const handleShare = async () => {
    ...
    const isPerPanelShare = !assessment.vehicle_id && assessment.vehicles && assessment.vehicles.length > 0;

    if ((assessment.is_multi_vehicle || isPerPanelShare) && assessment.vehicles && assessment.vehicles.length > 0) {
      shareText += `\n*Vehicles & Line Items:*\n\n`;
    ...
    const _sub = (assessment.is_multi_vehicle || isPerPanelShare) && assessment.vehicles?.length
      ? assessment.vehicles.reduce((s, v) => {
          ...
          return s + vTotal;
        }, 0) + (assessment.line_items || []).reduce((s, i) => s + ((i.quantity || 1) * (i.unit_price || 0)), 0)
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            {assessment.status === 'completed' && assessment.payment_status !== 'paid' && (
              <Button onClick={() => handlePaymentStatusChange('paid')} disabled={isUpdating} className="w-full bg-yellow-500 hover:bg-yellow-600 text-slate-900 font-semibold">
                {isUpdating ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <CheckCircle className="w-4 h-4 mr-2" />}
                Mark as Paid
              </Button>
            )}
